...
ARCHITECTURE vhdl OF slave_timing_6502 IS
	...
	signal phi_halt_n_reg : std_logic;

	signal halt_n_del_next : std_logic;
	signal halt_n_del_reg : std_logic;
begin
	...
	begin
		if (reset_n='0') then
			...
			halt_n_del_reg <= halt_n_del_next;
		end if;
	end process;
	...
	process(phi2_falling_edge_reg,phi_halt_n_reg,halt_n_del_reg)
	begin
		halt_n_del_next <= halt_n_del_reg;
		if (phi2_falling_edge_reg='1') then
			halt_n_del_next <= phi_halt_n_reg;
		end if;
	end process;
	...
	rw_n <= phi_rw_n_reg;	
	CS <= phi_cs_reg;
	halt_n_out <= halt_n_del_reg;

	data_cycle <= phi2_falling_edge;
