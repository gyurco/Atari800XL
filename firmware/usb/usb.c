...
uint8_t usb_set_address(usb_device_t *dev, ep_t *ep, 
  ...
  */
  ...
  usbhostslave[OHS900_TXADDRREG] = dev->bAddress;

  controlAdj = 0;
  if (bmHubPre)
  {
    controlAdj = OHS900_HCTLMASK_PREAMBLE_EN;
  }
...
uint8_t usb_dispatchPktWithData( uint8_t token, uint8_t ep, uint16_t nak_limit, uint8_t * data, uint16_t bytes_tosend, uint8_t * sndToggle) {
  ...
  while( timeout > timer_get_msec() )  {
    ...
    usbhostslave[OHS900_TXENDPREG] = ep;
    uint8_t control = OHS900_HCTLMASK_TRANS_REQ|OHS900_HCTLMASK_SOF_SYNC|controlAdj;
    uint8_t line_control = 0;
    uint8_t out = 0;
    switch (token)
    {
    case tokSETUP:
        usbhostslave[OHS900_TXTRANSTYPEREG] = OHS900_SETUP;
        break;
    case tokIN:
        usbhostslave[OHS900_TXTRANSTYPEREG] = OHS900_IN;
        break;
    case tokOUT:
        usbhostslave[OHS900_TXTRANSTYPEREG] = *sndToggle ? OHS900_OUT_DATA1 : OHS900_OUT_DATA0;
        out = 1;
        break;
    case tokINHS:
        usbhostslave[OHS900_TXTRANSTYPEREG] = OHS900_IN;
	line_control = OHS900_TXLCTL_MASK_FSPD;
        break;
    case tokOUTHS:
        usbhostslave[OHS900_TXTRANSTYPEREG] = *sndToggle ? OHS900_OUT_DATA1 : OHS900_OUT_DATA0;
        out = 1;
    ...
    if (out)
    {
      ...
      *sndToggle = !*sndToggle; // Correct place to flip Original code did this outside loop but that looks wrong - we should send 0/1/0/1 etc.
      ...
      while (--toSend)
      {
        usbhostslave[OHS900_HOST_TXFIFO_DATA] = *dataToSend++;
      }
    ...
    usbhostslave[OHS900_TXLINECTLREG] = line_control;
    usbhostslave[OHS900_HOST_TX_CTLREG] = control;
    ...
    while( timer_get_msec() < timeout )	{
      ...
      // MWW
      tmpdata = usbhostslave[OHS900_IRQ_STATUS];

      if( tmpdata & OHS900_INTMASK_TRANS_DONE ) {
	//clear the interrupt
	//max3421e_write_u08( MAX3421E_HIRQ, MAX3421E_HXFRDNIRQ );
	// MWW
	usbhostslave[OHS900_IRQ_STATUS] = OHS900_INTMASK_TRANS_DONE;
	rcode = 0x00;
	break;
      }
    }
    ...
    rcode = usbhostslave[OHS900_HRXSTATREG];
  ...
  return( rcode&~(OHS900_STATMASK_ACK_RXED|OHS900_STATMASK_DATA_SEQ));
}
...
uint8_t usb_InTransfer(ep_t *pep, uint16_t nak_limit, 
  ...
  *nbytesptr = 0;
  ...
  //    return ( 0xf0 );                            //receive error
    stat = usbhostslave[OHS900_HRXSTATREG];
    ...
    pktsize = usbhostslave[OHS900_RXFIFOCNTLSBREG];
    ...
    while (--toRead)
    {
      *data++ = usbhostslave[OHS900_HOST_RXFIFO_DATA];
    }
...
void usb_poll() {
  ...
  uint8_t conState = usbhostslave[OHS900_RXCONNSTATEREG];
  switch(conState)
  {
...
/*  switch( tmpdata )  {
  ...
  if(timer_get_msec() > poll) {
	...
	dev[i].class->poll(dev+i);
	// MWW (rcode unused error) rcode = dev[i].class->poll(dev+i);
	...
	if(dev[i].bAddress && dev[i].class) {
	  dev[i].class->release(dev+i);
	  // MWW (rcode unused error) rcode = dev[i].class->release(dev+i);
	  dev[i].bAddress = 0;
	}
      }
    ...
    case USB_ATTACHED_SUBSTATE_RESET_DEVICE:
      ...
      usbhostslave[OHS900_SOFENREG] = 0;
      usbhostslave[OHS900_TXLINECTLREG] = OHS900_TXLCTL_MASK_SE0;
      usb_task_state = USB_ATTACHED_SUBSTATE_WAIT_RESET_COMPLETE;
      delay = timer_get_msec() + 50; // send reset for 50msec
      break;
      
    case USB_ATTACHED_SUBSTATE_WAIT_RESET_COMPLETE:
      if( delay < timer_get_msec() )
      {
        usbhostslave[OHS900_SOFENREG] = OHS900_MASK_SOF_ENA;
        usbhostslave[OHS900_TXLINECTLREG] = OHS900_TXLCTL_MASK_NORMAL;
        usbhostslave[OHS900_IRQ_STATUS] = OHS900_INTMASK_SOFINTR;
    ...
    case USB_ATTACHED_SUBSTATE_WAIT_SOF:  //todo: change check order
      ...
      if (usbhostslave[OHS900_IRQ_STATUS]&OHS900_INTMASK_SOFINTR)
      {
        usbhostslave[OHS900_IRQ_STATUS] = OHS900_INTMASK_SOFINTR;
